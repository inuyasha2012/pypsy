...
class BaseUIrt(BaseIrt):
    ...
    def __init__(self, init_slop=None, init_threshold=None, max_iter=1000, tol=1e-5, *args, **kwargs):
        super(BaseUIrt, self).__init__(*args, **kwargs)
    ...
    def _m_step(self, full_dis, right_dis, slop, threshold, theta, p_val, z):
        # 一阶导数, 二阶导数
        dp, ddp = self._get_dp_n_dd(right_dis, full_dis, p_val, z)
        # jac矩阵和hess矩阵
...
class ProbitIrt(BaseUIrt):
    ...
    def _get_dp_n_dd(self, right_dis, full_dis, p_val, z):
        h = self._h(z)
        w = self._w(h, p_val)
...
class Irt(object):
    ...
    LINK = {'logit': LogitIrt, 'probit': ProbitIrt}

    def __init__(self, scores, link='logit', init_slop=None, init_threshold=None, max_iter=1000, tol=1e-5, *args, **kwargs):
        if link not in ('probit', 'logit'):
            raise ValueError('link must be probit or logit')
        _link = self.LINK[link]
